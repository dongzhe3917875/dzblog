(function($) {
  ...
  $.jqPaginator = function(el, options) {
    ...
    self.init = function() {
      ...
      // onPageChange的第一次执行
      self.fireEvent(this.options.currentPage, 'init', beforeIndex,
        endIndex);
    };
    ...
    self.verify = function() {
      ...
      if (opts.totalPages < 1) {
        throw new Error(
          '[jqPaginator] totalPages cannot be less currentPage');
      }
    };

    self.extendJquery = function() {
      $.fn.jqPaginatorHTML = function(s) {
        return s ? this.before(s).remove() : $('<p>').append(this.eq(0)
          .clone()).html();
      };
    };
    ...
    self.renderHtml = function() {
      ...
      self.isEnable('prev') && html.unshift(self.buildItem('prev', self.options
        .currentPage - 1));
      self.isEnable('statistics') && html.unshift(self.buildItem(
        'statistics'));
      if (endIndex) {
        html.push(self.buildItem("endIndex", "<span>...</span>"));
      }
      if (self.options.totalPages > 1) {
        self.isEnable('last') && html.push(self.buildItem('last', self.options
          .totalPages));
      }

      self.isEnable('next') && html.push(self.buildItem('next', self.options
        .currentPage + 1));
    ...
    self.setStatus = function() {
      ...
      $('[jp-role=page][jp-data=' + options.currentPage + ']', self.$container)
    ...
    self.fireEvent = function(pageIndex, type, beforeIndex, endIndex) {
      return (typeof self.options.onPageChange !== 'function') || (self.options
        .onPageChange(pageIndex, type, beforeIndex, endIndex) !== false
      );
    };
    ...
    self.callMethod = function(method, options) {
      switch (method) {
        ...
        case 'destroy':
          ...
          break;
        default:
          throw new Error('[jqPaginator] method "' + method +
            '" does not exist');
      }
